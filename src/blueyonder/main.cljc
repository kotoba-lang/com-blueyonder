(ns blueyonder.main
  "Kotodama WASM entrypoint for the Blueyonder clean-room actor (L4) — Clojure port.
  ...
  facts (`blueyonder.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "blueyonder")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"blueyonder.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "blueyonder-compat" :tier tier :entities entities} 200])
...
(defn start! [] :blueyonder-compat/ready)
